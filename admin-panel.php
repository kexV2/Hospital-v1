...
// VULNERABILITY FIX 2: Helper to escape output and prevent stored XSS
...
// VULNERABILITY FIX 1: Prevent IDOR on appointment cancellation
if (isset($_GET['cancel']) && isset($_GET['ID']) && isset($_SESSION['pid'])) {
    $cancel_id  = (int) $_GET['ID'];          // force integer
    $cancel_pid = (int) $_SESSION['pid'];     // current logged-in patient

    // Only cancel if this appointment belongs to the logged-in patient
    if ($stmt = mysqli_prepare($con, "UPDATE appointmenttb SET userStatus='0' WHERE ID = ? AND pid = ?")) {
        mysqli_stmt_bind_param($stmt, "ii", $cancel_id, $cancel_pid);
        ...
        if (mysqli_stmt_affected_rows($stmt) > 0) {
            ...
            echo "<script>alert('Unable to cancel this appointment.');</script>";
        }
...
function generate_bill(){
  ...
  $query = mysqli_query(
    $con,
    "select p.pid,p.ID,p.fname,p.lname,p.gender,p.email,p.contact,
            a.appdate,a.apptime,a.disease,a.allergy,a.prescription,
            a.docFees,a.doctor  
     from patreg p 
  ...
  while ($row = mysqli_fetch_array($query)) {
    ...
    // VULNERABILITY FIX 2: escape all user-controlled values
    $output .= '<label> Patient ID : </label>'        . e($row["pid"])         . '<br/><br/>';
    $output .= '<label> Appointment ID : </label>'    . e($row["ID"])          . '<br/><br/>';
    $output .= '<label> Patient Name : </label>'      . e($row["fname"]) . ' ' . e($row["lname"]) . '<br/><br/>';
    $output .= '<label> Doctor Name : </label>'       . e($row["doctor"])      . '<br/><br/>';
    $output .= '<label> Appointment Date : </label>'  . e($row["appdate"])     . '<br/><br/>';
    $output .= '<label> Appointment Time : </label>'  . e($row["apptime"])     . '<br/><br/>';
    $output .= '<label> Disease : </label>'           . e($row["disease"])     . '<br/><br/>';
    $output .= '<label> Allergies : </label>'         . e($row["allergy"])     . '<br/><br/>';
    $output .= '<label> Prescription : </label>'      . e($row["prescription"]). '<br/><br/>';
    $output .= '<label> Fees Paid : </label>'         . e($row["docFees"])     . '<br/><br/>';
  }
...
<div class="tab-pane fade" id="app-hist" role="tabpanel" aria-labelledby="list-pat-list">
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  ...
                  ?>
                      <tr>
                      <td><?php echo e($row['doctor']); ?></td>    <!-- VULNERABILITY FIX 2 -->
                      <td><?php echo e($row['docFees']); ?></td>  <!-- VULNERABILITY FIX 2 -->
                      <td><?php echo e($row['appdate']); ?></td>  <!-- VULNERABILITY FIX 2 -->
                      <td><?php echo e($row['apptime']); ?></td>  <!-- VULNERABILITY FIX 2 -->
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  ...
                  ?>
                      <tr>
                        <td><?php echo e($row['doctor']); ?></td>       <!-- VULNERABILITY FIX 2 -->
                          <td><?php echo e($row['ID']); ?></td>           <!-- VULNERABILITY FIX 2 -->
                          <td><?php echo e($row['appdate']); ?></td>      <!-- VULNERABILITY FIX 2 -->
                          <td><?php echo e($row['apptime']); ?></td>      <!-- VULNERABILITY FIX 2 -->
                          <td><?php echo e($row['disease']); ?></td>      <!-- VULNERABILITY FIX 2 -->
                          <td><?php echo e($row['allergy']); ?></td>      <!-- VULNERABILITY FIX 2 -->
                          <td><?php echo e($row['prescription']); ?></td> <!-- VULNERABILITY FIX 2 -->
                        <td>
                          <form method="get">
