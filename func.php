...
if ($_SESSION['loginAttempts'] >= $attempt_limit) {
    $remaining = $_SESSION['timeOut'] - time();
    if ($remaining > 0) {
...
if (isset($_POST['patsub'])) {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if ($_SESSION['loginAttempts'] >= $attempt_limit) {
            $_SESSION['timeOut'] = time() + $lockout_seconds;
            die("Too many failed attempts. You are locked out for $lockout_seconds seconds.");
        }
...
if (isset($_POST['doc_sub'])) {
    ...
    $query  = "INSERT INTO doctb(username,password,email,docFees)
               VALUES('$doctor','$hashedPassword','$demail','$docFees')";
    $result = mysqli_query($con, $query);
